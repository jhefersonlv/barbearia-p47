...
const MOCK_EMPLOYEES = [
  {
    id: "emp-001", name: "Júlio Castro", position: "Fundador & Mestre Barbeiro",
    specialty: "Geometria do Corte · Visagismo · Tricologia",
    initials: "JC", bgColor: "#2a1f0a", isFounder: true,
    workingHours: { enabled: true, timezone: "America/Sao_Paulo", workingDays: MOCK_WORKING_DAYS },
  },
  {
    id: "emp-002", name: "César", position: "Barbeiro Sênior",
    specialty: "Cortes Clássicos · Navalhação",
    initials: "CS", bgColor: "#0a1a2a", isFounder: false,
    workingHours: { enabled: true, timezone: "America/Sao_Paulo", workingDays: MOCK_WORKING_DAYS },
  },
  {
    id: "emp-003", name: "Yago", position: "Barbeiro",
    specialty: "Cortes Modernos · Degradê",
    initials: "YG", bgColor: "#1a0a2a", isFounder: false,
    workingHours: { enabled: true, timezone: "America/Sao_Paulo", workingDays: MOCK_WORKING_DAYS },
  },
  {
    id: "emp-004", name: "Rafael", position: "Barbeiro",
    specialty: "Barba & Bigode · Coloração",
    initials: "RF", bgColor: "#0a2a1a", isFounder: false,
    workingHours: { enabled: true, timezone: "America/Sao_Paulo", workingDays: MOCK_WORKING_DAYS },
  },
  {
    id: "emp-005", name: "Yasmin", position: "Barbeira & Visagista",
    specialty: "Visagismo · Sobrancelha · Limpeza de Pele",
    initials: "YS", bgColor: "#2a0a1a", isFounder: false,
    workingHours: { enabled: true, timezone: "America/Sao_Paulo", workingDays: MOCK_WORKING_DAYS_SAT_OFF },
  },
];
...
function generateAllSlots(emp, dayOfWeek, durationMin) {
  ...
  for (const ts of (wd.timeSlots || [])) {
    ...
    const end   = eh * 60 + em;
    for (let t = start; t + durationMin <= end; t += interval) {
...
async function mockApiCall(endpoint, options = {}) {
  ...
  if (endpoint.startsWith('/external/v1/booking/slots')) {
    ...
    const available = allSlots.filter(() => rand() > 0.3);
  ...
  if (endpoint === '/external/v1/booking' && options.method === 'POST') {
    ...
    return {
      ...
      employeeName: emp?.name || 'Profissional',
...
// ─── EXPÕE GLOBAIS PARA app.js ────────────────────────────────────────

window.MOCK_WORKING_DAYS         = MOCK_WORKING_DAYS;
window.MOCK_WORKING_DAYS_SAT_OFF = MOCK_WORKING_DAYS_SAT_OFF;
window.MOCK_SERVICES             = MOCK_SERVICES;
window.MOCK_EMPLOYEES            = MOCK_EMPLOYEES;
window.MOCK_TESTIMONIALS         = MOCK_TESTIMONIALS;
window.GALLERY_ITEMS             = GALLERY_ITEMS;
window.seededRandom              = seededRandom;
window.dateToSeed                = dateToSeed;
window.generateAllSlots          = generateAllSlots;
